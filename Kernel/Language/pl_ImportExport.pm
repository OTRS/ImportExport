...
# --
# $Id: pl_ImportExport.pm,v 1.10.2.1 2011-12-19 16:20:23 ub Exp $
# --
...
sub Data {
    ...
    $Self->{Translation}->{'Semicolon (;)'} = '¦rednik (;)';
    $Self->{Translation}->{'Tabulator (TAB)'} = 'Tabulator (TAB)';
    $Self->{Translation}->{'Include Column Headers'} = '';

    # Template: AdminImportExport
    $Self->{Translation}->{'Import/Export Management'} = 'Zarz±dzanie Importem/Exportem';
    ...
    $Self->{Translation}->{'Delete Template'} = 'Usuñ szablon';
    ...
    $Self->{Translation}->{'Source File'} = 'Plik ¼ród³owy';

    # SysConfig
    $Self->{Translation}->{'Format backend module registration for the import/export module.'} = 'Modu³ formatowania backend dla modu³u import/eksport.';
    ...
    $Self->{Translation}->{'Maximum number of one element'} = 'Maksymalna liczba wyst±pieñ pojedynczego elementu';
    $Self->{Translation}->{'Empty fields indicate that the current values are kept'} = 'Puste pola oznaczaj± zachowanie bie¿±cej warto¶ci';
    $Self->{Translation}->{'Element required, please insert data'} = 'Element wymagany, podaj warto¶æ';
